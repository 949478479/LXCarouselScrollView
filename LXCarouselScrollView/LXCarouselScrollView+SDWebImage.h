...
/// 网络图片地址数组
@property (nullable, nonatomic, setter=lx_setImageURLs:) NSArray<NSURL *> *lx_imageURLs;

/// 在下载图片过程中是否显示活动指示器，默认不显示
@property (nonatomic, setter=lx_setShowActivityIndicator:) BOOL lx_showActivityIndicator;
/// 在下载图片过程中显示的占位图片，设置该属性将不再显示活动指示器
@property (nullable, nonatomic, setter=lx_setPlaceholderImage:) UIImage *lx_placeholderImage;
